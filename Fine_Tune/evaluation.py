...
def centroid_affinity(
    ...
    def mean_embed(lines):
        ...
        return np.mean(
            st.encode(lines, convert_to_numpy=True, show_progress_bar=False), axis=0
        )
    ...
    for char, lines in gen_lines.items():
        for line in lines:
            ...
            sims = {
                c: float(cosine_similarity([emb], [centroids[c]])[0, 0])
                for c in centroids
            }
            ...
            results.append(
                {
                    "speaker": char,
                    "predicted_as": best,
                    "cosine_to_true": sims.get(char, 0.0),
                }
            )
    ...
    df = (
        ...
        if results
        else pd.DataFrame(columns=["speaker", "predicted_as", "cosine_to_true"])
    )
    acc = float((df["speaker"] == df["predicted_as"]).mean()) if not df.empty else 0.0
    confusion = (
        pd.crosstab(df["speaker"], df["predicted_as"], normalize="index")
        if not df.empty
        else pd.DataFrame()
    )
    per_char_cos = (
        df.groupby("speaker")["cosine_to_true"].mean()
        if not df.empty
...
_feat_regex = {
    ...
    "modal_verbs": re.compile(
        r"\b(can|could|should|might|must|may|would|will)\b", re.I
    ),
...
def main():
    ...
    ap.add_argument(
        "--model_dir",
        type=str,
        default="/Users/ransela/merged",
        help="Path to merged HF model folder",
    )
    ap.add_argument(
        "--test_path",
        type=str,
        default="bbt_test.jsonl",
        help="Path to JSONL with fields: prompt, target, target_speaker",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Limit test samples for quick runs",
    )
    ...
    # Load test items
    test_path = "./Fine_Tune/fine_tune_data/bbt_test_cleaned.jsonl"
    ...
    for model in ["original", "Fine_tune"]:
        if model == "original":
            ...
            path_to_gen = "./Fine_Tune/fine_tune_data/output_files/gens_orig_llama.jsonl"
        else:
            args.model_dir = "/Users/ransela/merged"
            path_to_gen = "./Fine_Tune/fine_tune_data/output_files/gens_fine_tune.jsonl"
        ...
        acc, per_char_cos, confusion, df_aff = centroid_affinity(
            real_lines, gen_lines, emb_model_name=args.emb_model
        )
        ...
        summary_rows.append({"centroid_affinity_acc": acc, "jsd_overall": jsd_overall})
